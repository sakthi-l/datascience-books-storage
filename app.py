...
def search_books():
    ...
    if keywords: query["keywords"] = {"$in": [k.strip() for k in keywords.split(",")]}
    ...
    for book in books:
        with st.expander(book["title"]):
            ...
            if st.button(f"📖 View PDF – {book['file_name']}", key=f"view_{book['_id']}"):
                pdf_link = f'<iframe src="data:application/pdf;base64,{book['file_base64']}" width="100%" height="600"></iframe>'
                st.markdown(pdf_link, unsafe_allow_html=True)

            # Download (only if logged in)
            user = st.session_state.get("user")
            if user and user != "admin":
                st.download_button(
                    "📥 Download Book",
                    base64.b64decode(book["file_base64"]),
                    book["file_name"],
                ...
                st.warning("🔐 Login to download or bookmark.")
